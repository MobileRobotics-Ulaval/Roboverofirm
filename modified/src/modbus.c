...
int _mobusInteract(uint8_t * args){
    ...
    //unsigned char maphrase[25] = "ma langue est ridicule\n";
    //return writeMultiDataRegisterLeddar(arguments[0], 0x0, maphrase, sizeof(maphrase));

    I2CAdd = arguments[0];
    //LtU16 lValues[10];
    //ModbusReadInputRegisters( 20, 10, lValues );
    //LtByte dummy = 3;
    //LtResult  res = ModbusConnect("", 1);

    //LtResult res = ModbusSend( MODBUS_SERVER_ID, NULL, 0 );
    //LtByte lDummy[MODBUS_MAX_PAYLOAD];
    //res = ModbusReceive( lDummy );
    LtResult res = 0;
...
/*// ---- start debug connect --- 
    // res = ModbusConnect("", 1); //01 11 C0 2C
    res = ModbusSend( MODBUS_SERVER_ID, NULL, 0 );
    ...
    if ( res == LT_SUCCESS )
    {
        ...
        //int j = sizeof(lMessage);
        lRead = 2;//ReadFromSerialPort( gHandle, lMessage, sizeof(lMessage) );
            ...
            if ( lRead >= MODBUS_OVERHEAD )
            {
                if ( CRC16( lMessage, lRead-MODBUS_CRC_LEN, LT_TRUE ) )
                {
                         ...
                         && ( lMessage[1] == gLastFunction ) )
                    {
                        ...
                        if ( lDummy != NULL )
                        {
                            memcpy( lDummy, lMessage+MODBUS_HEADER_LEN, lRead-MODBUS_OVERHEAD );
                        }
                        else
                            res = lRead-MODBUS_OVERHEAD;
                    }
                    else
                        res = LT_PROTOCOL_ERROR;
                }
                else
                    res = LT_BAD_CRC;
            }
...
LtResult WriteToSerialPort( LtHandle aHandle, LtByte *aData, int aLength ){
    writeMultiDataRegisterLeddar(I2CAdd, LEDDARONE_DLL, aData, aLength);
...
LtResult ReadFromSerialPort( LtHandle aHandle, LtByte *aData, int aMaxLength )
{
    LtResult lRead = 0;
    int dataInFIFO = 0;
    int bytesReceive = 400; //0;
    /*while(bytesReceive < aMaxLength ){
        // We read the number of bytes in FIFO
        dataInFIFO = 1;//readRegisterLeddar(I2CAdd, LEDDARONE_RXLVL);
        // If we have more data in fifo than we need we only read what we need
        if(bytesReceive + dataInFIFO > aMaxLength)
             dataInFIFO = aMaxLength - bytesReceive;
        // We fetch one byte at a time
        while(dataInFIFO > 0){
            //aData[bytesReceive] = readRegisterLeddar(I2CAdd, LEDDARONE_DLL);
            bytesReceive++;
            dataInFIFO--;
        }
    }*/
